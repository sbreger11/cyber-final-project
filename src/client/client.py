...
search [pattern1] [pattern2] ... - Type this command to search for n patterns in the message logs. n = 1 will use
          Boyer moore & n > 1 will use Aho Corasick.
...
def handle_server(client_socket):
    ...
    try:
            ...
            while not shutdown_event.is_set():
                ...
                if user_input == "broadcast":
                    ...
                    if len(split_user_input) >= 2 and split_user_input[0] == "search":
                        ...
                        if split_user_input[0] == "show" and split_user_input[1] == "messages" and split_user_input[2].isdigit():
        ...
        logger.error("server {(server_ip, server_port)} had an error: {e}\ndisconnecting from {(server_ip, server_port)}")
    finally:
